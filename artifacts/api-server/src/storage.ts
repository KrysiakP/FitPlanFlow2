...
export interface IStorage {
  ...
  bulkImportExerciseLibrary(exercises: { name: string; category?: string }[], trainerId: string): Promise<{ imported: number; skipped: number }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async bulkImportExerciseLibrary(exercises: { name: string; category?: string }[], trainerId: string): Promise<{ imported: number; skipped: number }> {
    ...
    if (toInsert.length > 0) {
      await db.insert(exerciseLibrary).values(
        toInsert.map(({ name, category }) => ({ name, category: category ?? null, trainerId }))
      );
    }
    return { imported: toInsert.length, skipped: exercises.length - toInsert.length };
  }
