...
- **Drawer hamburger menu** — Shared `components/DrawerMenuButton.tsx` component; trainer screens with custom headers (`index`, `diets`, `chat`, `profile`) have `headerShown: false` in `(trainer)/_layout.tsx` and embed `DrawerMenuButton` in their own header rows; other trainer screens (`exercise-library`, `invitations`, `payments`, `referrals`) keep navigation header with built-in hamburger; `topPad` in custom-header screens changed from `Platform.OS === "web" ? 67 : insets.top` to just `insets.top`
- **App Store readiness** — icon upgraded to 1024×1024; adaptive icon (transparent bg) for Android added; `ITSAppUsesNonExemptEncryption: false` added to iOS infoPlist; iOS Privacy Manifest (`NSPrivacyManifests`) added with required API categories (UserDefaults, FileTimestamp, DiskSpace, SystemBootTime); splash screen bg synced to `#0d0d0d`
