...
export default function MyProgress() {
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Śledź swoje postępy w treningu i transformacji
          </p>
        </div>
        ...
        <Alert>
          ...
          <AlertDescription>
            Wypełnij swój pierwszy raport tygodniowy, aby zobaczyć swoje postępy
          </AlertDescription>
        </Alert>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      ...
      {oldestReport && newestReport && oldestReport.id !== newestReport.id && (
        <Card>
          <CardHeader>
            <CardTitle className="font-heading flex items-center gap-2">
              <ImageIcon className="w-5 h-5" />
              Transformacja - Zdjęcia przed i po
            </CardTitle>
            <CardDescription>
              Porównaj swoje zdjęcia z {format(new Date(oldestReport.reportDate), "d MMMM yyyy", { locale: pl })} 
              {" i "}
              {format(new Date(newestReport.reportDate), "d MMMM yyyy", { locale: pl })}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="font-heading font-semibold text-lg">Przed</h3>
                  <Badge variant="outline">
                    {format(new Date(oldestReport.reportDate), "dd.MM.yyyy")}
                  </Badge>
                </div>
                {oldestReport.photoUrl ? (
                  <div className="aspect-video rounded-lg border overflow-hidden bg-muted">
                    <img
                      src={oldestReport.photoUrl}
                      alt="Zdjęcie przed"
              ...
              <div className="space-y-3 md:col-start-2">
                <div className="flex items-center justify-between">
                  <h3 className="font-heading font-semibold text-lg">Po</h3>
                  <Badge variant="default">
                    {format(new Date(newestReport.reportDate), "dd.MM.yyyy")}
                  </Badge>
                </div>
                {newestReport.photoUrl ? (
                  <div className="aspect-video rounded-lg border overflow-hidden bg-muted">
                    <img
      ...
      {exerciseLogs && exerciseLogs.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Twoje najlepsze wyniki w poszczególnych ćwiczeniach
            </CardDescription>
          </CardHeader>
          <CardContent>
